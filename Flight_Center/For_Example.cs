...
namespace Flight_Center
{
    class For_Example
    {
        ...
        /*   Ниже приведена генерическая функция, кот принимает "conn_string", имя Stored Procedure и её параметры ввиде массива даных.
         *   Считывание данных происходит через READER. Эти данные записываются в  Dictionary<string, object>, где:
         *      1.string - это название отдельного столбца таблицы[ColumnName] 
         *      2.object -- это принадляжащие ему записи всех строк таблицы [column_value] построчно. 
         *   READER за одну итерацию считывает данные  переходя от столбца к столбцу данной таблицы и последовательно "вынимает"
         * из них их значения. Таким образом за одну итерацию READER cчитывает данные из одной строки соответствено ёё столбцам. 
         *   Все эти данные формируются как отдельный объект с названием one_row (one_row.Add(item.ColumnName, column_value)
         *   Отсюда объект one_row -- это полный прототип строки данной таблицы. 
         *   Объект one_row записывается в List<Dictionary<string, object> как его отдельный член (ITEM). Таким образом,по завершению
         * цикла ==>  List<Dictionary<string, object> будет содержать в себе все строки таблицы ввиде отдельных своих ITEM-ов.
         *   List<Dictionary<string, object> -- это полный прототип данной  таблицы.
         */
